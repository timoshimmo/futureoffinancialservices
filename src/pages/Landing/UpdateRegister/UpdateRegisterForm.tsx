...
const UpdateRegisterForm = () => {
    ...
    const [userData, setUserData] = useState<EventProps>({
        id: '',
        first_name: '',
        last_name: '',
        email: '',
    ...
    const validation = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            workshops: Yup.array().min(1, "Please you are required to select at least one roundtable session to attend").max(2, "You can only select maximum of two roundtable sessions")
        }),
        onSubmit: (values) => {
           
            let yFilter = values.workshops.map((itemY: any) => { return itemY });
            const eventList = workshopsData.filter(item => yFilter.some((data: any) => data === item.event_name));

            const valuesObj = {...values} as Partial<any>;
            delete valuesObj?.workshops;

            let trimObj = eventList.map(data => {
                const rndId = Math.floor(Math.random() * 101);
                const objVal = {...data, id: rndId, registrationId: userData.id} as Partial<any>;
    ...
    const handleGetUser = () => {
        //http://localhost:4005/api/v1/register/531bb96e-8411-47e0-9f7d-4a2cd6e2bd8b
        if(token !== null || token !== "") {
            axios.get(`https://dev-api.futureoffinancialservices.org/api/v1/register/${token}`)
            .then(response => {
    ...
    const handleClick = (obj: any) => {
       ...
       /* setTimeout(() => {
        ...
        axios.put(`https://dev-api.futureoffinancialservices.org/api/v1/register/${userData.id}`, obj)
        .then(response => {
                                    ...
                                    <CardBody className="p-4">
                                    {
                                        ...
                                        <Row className='justify-content-center'>
                                                <Col lg={9} sm={12} className='px-2 mt-3'>
                                                    ...
                                                    <h2 className='text-primary my-3 text-center fw-bold' style={{ fontFamily: 'Georgia, "Times New Roman", Times, serif' }}>You have already successfully registered to our workhops</h2>
                                                    <p className='text-dark fs-14 text-center fw-medium px-4 mb-4' style={{ color: '#303030' }}>Your request is currently under review, we will get back to you shortly</p>
